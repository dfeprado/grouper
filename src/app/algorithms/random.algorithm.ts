...
*/
export class Cyrb128Sfc32RandomAlgorithm {
  public static seed(seed: string) {
    const instance = new Cyrb128Sfc32RandomAlgorithm(seed);
    return (ceiling: number): number => {
      return instance.random(ceiling);
    };
  }
  ...
  private constructor(seed: string) {
    const seedVector = this.cyrb128(seed);
    this.randomFn = this.sfc32(seedVector);
  }
